...
void line_evaluation() {
	...
	if (greendots[0] == cv::Point(0, 0) && greendots[1] == cv::Point(0, 0)) {  //drive straight
		...
		std::cout << "Hallo12.3 " << std::endl;   //BUG SOMEWHERE  (probably only one Point in Vertex or so causing problems)
		int minpos = 0;
		int mindist = dists[0];
	...
	if (greendots[0] != cv::Point(0, 0) || greendots[1] != cv::Point(0, 0)) {  //turn to that point
		...
		if (forks.size() > 0) {
			...
			float w_2 = 2;
			int likely = 0;
			if (Vertecies[forks[fork].x][forks[fork].y][0].x < (greendots[1] != cv::Point(0, 0)) ? greendots[1].x : greendots[0].x) {
				//left
				for (int i = 0; i < destinations.size(); i++) {
					cv::Point da = (greendots[1] != cv::Point(0, 0)) ? greendots[1] : greendots[0]; //entsprechende grüner punkt
					cv::Point db = destinations[i];
					cv::Point dc = destinations[likely];
