...
//25.05.20		: Removal of old Green Dots and full change to Green Dot Struct
//					--> left and right not working properly
//26.05.2020	: Left and right not working fixed by assigning negative perc when ofscreen
//===========================================================================================
...
float get_direction(std::vector<GreenDot> g_dots, std::vector<float> b_angles, int& index_);
int get_instructions(float dir);
void show(std::vector<std::vector<cv::Point>>& contours, std::vector<GreenDot>, int index);
void show();
...
//			std::vector<vector<cv::Point>> green_contours = get_green_dots(img_gre, green_dots);	//maybe also parse the green_dots by reference  
			...
			std::vector<GreenDot> green_dots = get_green_dots_struct(img_gre);						//replaces above function
...
//			std::vector<float> green_angles = get_angles(green_contours);
			...
			for (int i = 0; i < black_angles.size(); i++) {
				cout << "Black Angle " << i << ": " << black_angles[i] << std::endl;
			}

			//--direction------------------------
			int contours_index;
			float direction = get_direction(green_dots, black_angles, contours_index);			//from 90 -> left  to -90 -> right  (exept 180 -> 180° turn)
			...
			//--show-----------------------------		
			std::cout << "index" << contours_index << std::endl;
			show(contours, green_dots, contours_index);		
//			show();	//TEMP
		}
		else {		//--rote Zone----------------
...
std::vector<GreenDot> get_green_dots_struct(cv::Mat img) {
	...
	for (int i = contours.size() - 1; i >= 0; i--) {							//sees of the green dots are of relevance		(contours.size() cant be bigger than 2)
		...
		for (int j = 0; j < 4; j++) {
			...
			float count = 0;												//needs to be a float so that count / h can equal -1
			for (int k = 0; k < h; k++) {
				cv::Point2f p = cv::Point(s.x + v.x*k, s.y + v.y*k);
				if (p.x < 0 || p.x >= img.cols || p.y < 0 || p.y >= img.rows) {
					if (k < 10)
						count = -h;											//if there are less than 10 pixels before it goes of screen -> give perc value of -1  (-h/h = -1)
		...
		if (perc[0] < .7 || perc[2] > 0.3) {								//if on top less than 70% or beneath more than 30% are black -> not of relevance	(check if perc[0] != -1 not needed since greendot wood be a top of screen and therefor not relevant anyway
			contours.pop_back();
			g_dots.pop_back();
			std::cout << "Not Relevant" << std::endl;
			continue;
		}
		if (perc[1] > .7 || perc[3] < 0.3 && perc[3] >= 0) {								//if right more than 70% or left less than 30% -> right
			cur_d->left = false;
		}
		else {
			cur_d->left = true;
...
std::vector<float> get_angles(std::vector<std::vector<cv::Point>> c) {
	...
	for (int i = 0; i < c.size(); i++) {
		...
		a.push_back(acos((p.x-cols/2) / sqrt(pow(p.x-cols/2,2)+pow(rows-p.y,2))) * 180 / PI);		//angel between 0 - 180 (180--links--90--rechts--0)
	//	cout << i << "Pos: " << p << "  angle: " << a[i] << endl;									///
	}
	return a;
}

float get_direction(std::vector<GreenDot> g_dots, std::vector<float> b_angles, int& index_) {		//index_ only for show
	if (g_dots.size() > 1) {				//180° turn
	...
	if (g_dots.size() != 0) {		//if one green dot
		int index = 0;
		if (g_dots[0].left) {
			float a = 0;
		...
		std::cout << "Angle Index: " << index << std::endl;
		index_ = index;						//only for show()
		return b_angles[index] - 90;
	}

	int index = 0;								//if there are no green dots drive as straight as posible
	float angle = 180;
	for (int i = 0; i < b_angles.size(); i++) {
...
void show(std::vector<std::vector<cv::Point>>& contours, std::vector<GreenDot> g_dots, int index) {
