...
std::vector<GreenDot> get_green_dots_struct(cv::Mat img) {
	...
	for (int i = contours.size()-1; i >= 0; i--) {							//sees of the green dots are of relevance		(contours.size() cant be bigger than 2)
		...
		std::cout << "GreenDot: " << i << "  size " << cur_d->size << "  center" << cur_d->center << "  corners: 0 " << cur_d->corners[0] << "  1 " << cur_d->corners[1] << "  2 " << cur_d->corners[2] << "  3 " << cur_d->corners[3] <<::endl;
...
void getRect(std::vector<cv::Point>& c	) {		//returns rectangle for greendots
	...
	for (int i = 0; i < v.size(); i++) {
		...
		std::cout << "angle: " << angles[i] << " length: " << sidelengths[i] << " vectors: " << v[i] << "  " << v[(i + 1) % v.size()] << std::endl;
	}
	for (int i = 0; i < angles.size(); i++) {	//PRINT
		std::cout << "angle: " << angles[i] << " length: " << sidelengths[i] << " corners: " << c[i] << " vectors: " << v[i] << "  " << v[(i + 1) % v.size()] << std::endl;
	}											//PRINT
