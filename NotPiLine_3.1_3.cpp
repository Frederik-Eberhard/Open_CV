			...
			if (DotsReal[x][y].x != 0 && DotsReal[x][y].y != 0) {
				...
				if (y != 0) {
					if (DotsReal[x][y - 1] != cv::Point(0, 0)) {  //top
						...
						if (similar(get_borderpos(x, y, 0),get_borderpos(x, y - 1, 2))) {
...
void repeat(int x, int y) {
	...
	if (TempLinks[x][y].size() != 0) {
		...
		for (int i = 0; i < TempLinks[x][y].size(); i++) {
			std::cout << "Hallo9.1.2 " << i << " " << TempLinks[x][y].size() << std::endl;
			linesstart[amount] = DotsReal[x][y];
			std::cout << "Hallo9.1.2.1 " << TempLinks[x][y][i] << std::endl;
			linesend[amount] = DotsReal[TempLinks[x][y][i].x][TempLinks[x][y][i].y];
			std::cout << "Hallo9.1.2.2 " << std::endl;
			temppoint = TempLinks[x][y][i];
			std::cout << "Hallo9.1.3 " << std::endl;
			for (int j = 0; j < TempLinks[TempLinks[x][y][i].x][TempLinks[x][y][i].y].size(); j++) {
				if (TempLinks[TempLinks[x][y][i].x][TempLinks[x][y][i].y][j] == cv::Point(x, y)) {
					std::cout << "Hallo9.1.4 " << std::endl;
					TempLinks[TempLinks[x][y][i].x][TempLinks[x][y][i].y].erase(TempLinks[TempLinks[x][y][i].x][TempLinks[x][y][i].y].begin() + j);
				}
			}
...
float get_median_thickness(int x, int y, bool direction) {  //true = height  false = width   //WORK IN PROGRESS
...
int* sort(int arr[]) {
	int count = 0;
	//int size = arr[].size();//sizeof(arr) / sizeof(*arr);  //gets size of array
