...
void link_clean() {												//important for fork_detection     
	std::cout << "Hallo11.1 link_cleaning" << std::endl;
	for (int x = 0; x < ANZAHL; x++) {
		for (int y = 0; y < ANZAHL; y++) {
			for (int i = 0; i < Links[x][y].size(); i++) {
	...
	std::cout << "Hallo11.2  cleaning done " << std::endl;
}
...
void get_startPoint(cv::Point& sP) {
	...
	std::cout << "StartPoint: " << sP << std::endl;
}
...
void line_evaluation() {
	...
	if (greendots[0] == cv::Point(0, 0) && greendots[1] == cv::Point(0, 0)) {  //drive straight
		...
		std::cout << "Hallo12.3 " << "dists size: " << dists.size() << std::endl;   //BUG SOMEWHERE  (probably only one Point in Vertex or so causing problems)  -> dists.size() == 0  ,probably Vertecies.size() == 0
		int minpos = 0;
		int mindist = dists[0];
	...
	if (greendots[0] != cv::Point(0, 0) || greendots[1] != cv::Point(0, 0)) {  //turn to that point
		std::cout << "Hallo12.5 " << std::endl;
		vector<cv::Point> forks;			//number of fork					 //, [forkpos][forkexit1][forkexit2]...
		get_dest_forks(Vertecies,forks);							//BUG here  (Vertecies.size == 0) <- not the reason
		std::cout << "dest_forks " << forks << std::endl;
		if (forks.size() > 0) {
			std::cout << "Hallo12.6 " << std::endl;
			...
			std::cout << "Hallo12.7 " << forks[fork] << std::endl;
...
void get_line_in_Vertecies(vector<vector<vector<cv::Point>>>& vertecies, vector<cv::Point>& line, cv::Point currentPoint) {
	vector<cv::Point> empty;
	std::cout << "Hallo12.5 " << currentPoint << std::endl;
	for (int i = vertecies[currentPoint.x][currentPoint.y].size() - 2; i >= 0; i--) {
		std::cout << "Hallo12.5.1 " << i << "  " << vertecies[currentPoint.x][currentPoint.y][i]  << std::endl;
...
void get_dest_forks(vector<vector<vector<cv::Point>>>& vertecies, vector<cv::Point>& forks) {   //BUG
	std::cout << "Hallo12.5.1 " << std::endl;
	vector<cv::Point> empty;
	bool next = false;
	for (int i = vertecies.size() - 1; i > 0; i--) {
		std::cout << "Hallo12.5.2 " << std::endl;
		for (int j = vertecies[i].size() - 1; j > 0; j--) {
			std::cout << "Hallo12.5.3 " << std::endl;
			if (vertecies[i][j] != empty) {
				next = false;
				if (j > 0) {
					std::cout << "Hallo12.5.4 " << std::endl;
					if (vertecies[i][j - 1] == empty) {  //part of fork
						for (int a = 0; a < forks.size(); a++) {
							std::cout << "Hallo12.5.6 " << std::endl;
							std::cout << "Vertecies " << std::endl;
							std::cout << "Forks " << std::endl;
							if (vertecies[i][j][0] == forks[a]) {
								next = true;
							}
							std::cout << "Hallo12.5.6.1 " << std::endl;
						}
						if (!next) {
							std::cout << "Hallo12.5.7 " << std::endl;
							forks.push_back(vertecies[i][j][0]);
						}
					}
				}
				else {		//if fork from startPoint
					std::cout << "Hallo12.5.8 " << std::endl;
					if (i > 0) {
						for (int a = 0; a < forks.size(); a++) {
							std::cout << "Hallo12.5.9 " << std::endl;
							if (vertecies[i][j][0] == forks[a]) {
								next = true;
							}
						}
						if (!next) {
							std::cout << "Hallo12.5.10 " << std::endl;
							forks.push_back(vertecies[i][j][0]);
						}
					}
				}
	...
	for (int i = 0; i < vertecies.size() - 1; i++) {
		for (int j = 0; j < vertecies[i].size(); j++) {
			std::cout << "Hallo12.5.2 " << std::endl;
			next = false;
			if (vertecies[i][j][0] == vertecies[i + 1][j][0]) {
				std::cout << "Hallo12.5.3 " << std::endl;
				for (int a = 0; a < forks.size(); a++) {
					std::cout << "Hallo12.5.4 " << std::endl;
					if (forks[a] == cv::Point(i, j)){
						std::cout << "Hallo12.5.5 " << std::endl;
						next = true;
					}
				}
				if (!next) {
					std::cout << "Hallo12.5.6 " << std::endl;
					forks.push_back(cv::Point(i, j));
				}
			}
		}
	}*/
