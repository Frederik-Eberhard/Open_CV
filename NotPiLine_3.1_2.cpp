...
int get_median_thickness(int x, int y, bool direction);
...
int get_median_thickness(int x, int y, bool direction) {  //true = height  false = width   //WORK IN PROGRESS
	...
	if (values.size() % 2 == 0) {  // if even
		return 1 / 2 * (values[values.size() / 2] + values[values.size() / 2 + 1]);  //nullcount for skipping all 0
	}
	else {  //if odd
		return values[(values.size() + 1) / 2];   //nullcount for skipping all 0
	}
}
