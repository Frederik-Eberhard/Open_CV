...
void sort_vertecies(vector<vector<cv::Point>>& final_verts, cv::Point lastVertexEnd, cv::Point lastPosition, vector<vector<vector<cv::Point>>>& vertecies);
...
void get_startPoint(cv::Point& sP) {
	sP = cv::Point(2, 4);				//TEMPORARY SOLLUTION -> NEEDS TO BE REWORKED
}
...
void loops() {
	...
	std::wcout << "final_verts: " << std::endl;
	for (int i = 0; i < final_verts.size(); i++) {
