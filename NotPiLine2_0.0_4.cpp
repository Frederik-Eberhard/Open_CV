...
cv::Mat img_edg;	//edges in the image
...
cv::Mat get_img_edg(cv::Mat img,cv::Mat img_b);
...
int main() {
	...
	cv::namedWindow("EDG", WINDOW_NORMAL);

	lineProgramm = false;		//starts with line-driving
	while (1) {
		...
		if (lineProgramm) {
			...
			//--black-binarisation---------------				
			cv::cvtColor(img_rgb, img_roi, cv::COLOR_BGR2GRAY);		//rough approximation of the black line
			cv::inRange(img_roi, 0, 50, img_roi);

			img_roi -= img_gre;									//subtracting green image from line image (roi)

			img_edg = get_img_edg(img_rgb, img_roi);
			//--get-green-dots-------------------
...
cv::Mat get_img_edg(cv::Mat img,cv::Mat img_b) {
	cv::createTrackbar("lower Edge", "EDG", &lslider, 500);
	cv::createTrackbar("diff", "EDG", &diff, 500);
	cv::Mat edges;
	cv::Mat e;
	
	cv::pyrDown(img, edges, Size(img.cols / 2, img.rows / 2));
	cv::pyrUp(edges, e, Size(img.cols, img.rows));
	cv::Canny(e, edges, 38, 38*3, 3, true);		//slider set to 38 does the trick quite nicely	(lslider)
	cv::pyrDown(edges, e, Size(img.cols / 2, img.rows / 2));
	cv::pyrUp(e, edges, Size(img.cols, img.rows));
	
	//for flood fill: get actual black pixel to fill from and needs preferably close to start pixel of last image
	floodFill(edges, cv::Point(img.cols/2- 50, img.rows-1), Scalar(255));
	return edges;
}
