	...
	for (int ix = 0; ix < ANZAHL; ix++) {
		for (int iy = 0; iy < ANZAHL; iy++) {
			...
			for (unsigned int i = 0; i<contours_arr[ix][iy].size(); i++) {
				...
				if (m.m00 > grocont) {
					grocont = m.m00;
					if (grocont > roisize / 10) { //wie viel des Felds soll die Kontur belegen, bevor sie gez�hlt wird
...
void find_greendot() {
	...
	for (int i = 0; i < 2; i++) {  //WORK IN PROGRESS
		if (greendots[i] != cv::Point(0, 0)) {
			std::cout << "Hallo8.3.1 " << std::endl;
			int count_black = 0;
			cv::Moments m = cv::moments(contours_green[i][index_green[i]]);
			double height = sqrt(m.m00);
			std::cout << "Hallo8.3.2 " << m.m00 << "  " << height << std::endl;
			for (int iy = 0; iy < height; iy++) {
				int x = greendots[i].x;
				int y = greendots[i].y - iy;
				std::cout << "Hallo8.3.2.1 " << x << " " << y << std::endl;
